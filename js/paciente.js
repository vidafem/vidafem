...
function fetchDoctorVacationState_(options) {
    ...
    doctorVacationRequest_ = fetch(API_URL + "?t=" + timestamp, {
        method: "POST",
        body: JSON.stringify({ action: "get_my_doctor_vacation", requester: currentPatientId })
    })
        .then(r => r.json())
        .then((res) => {
...
function showDoctorVacationModal_(vac) {
    ...
    if (msgEl) msgEl.innerText = String((vac && vac.mensaje) || "Tu medico se encuentra fuera temporalmente.");
...
window.onInfographicAction = function(type, idPost) {
    ...
    if (type === "fuente") {
        ...
        if (!url) {
            notify("Esta publicacion no tiene una fuente externa configurada.", "info");
            return;
        }
    ...
    if (type === "contacto") {
        ...
        if (!number) {
            notify("No hay telefono de WhatsApp configurado para este medico.", "warning");
            return;
        }
...
function checkPromoAndDashboard() {
    ...
    const promoPromise = fetch(API_URL + "?t=" + timestamp, { method: "POST", body: JSON.stringify({ action: "get_active_promotion", requester: currentPatientId }) }).then(r => r.json());
    ...
    const infographicPromise = fetch(API_URL + "?t=" + timestamp, { method: "POST", body: JSON.stringify({ action: "get_patient_infographics", requester: currentPatientId }) }).then(r => r.json());
    ...
    formResch.addEventListener('submit', function(e) {
        ...
        .then((vac) => {
            if (!vac || vac.loading || vac.error) {
                notify((vac && vac.error) || "No se pudo verificar si tu medico esta disponible.", "warning");
                throw new Error("vacation_check_failed");
            }
            if (vac.active) {
                showDoctorVacationModal_(vac);
                notify(vac.block_message || "Tu medico no esta tomando nuevas citas por vacaciones.", "warning");
