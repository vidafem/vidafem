...
function notifyAdminMutationResult_(res, successMessage, warningMessage) {
  const hasWarning = !!(res && res.warning);
  if (window.showToast) {
    window.showToast(
      hasWarning
        ? String(warningMessage || (successMessage + " con advertencia de sincronización."))
        : String(successMessage || "Operacion completada."),
  ...
  if (hasWarning) {
    alert(String(successMessage || "Operacion completada.") + "\nAdvertencia: " + String(res.warning || ""));
    return;
  }
...
function normalizePatientSearchTerm_(value) {
  return String(value || "").trim().toLowerCase();
}

function extractPatientNumericOrderKey_(patient) {
  const rawId = String((patient && patient.id_paciente) || "").trim();
  const idMatch = rawId.match(/(\d{6,})/);
  return idMatch ? Number(idMatch[1]) : 0;
}

function extractPatientDateOrderKey_(patient) {
  const rawDate = String((patient && patient.fecha_registro) || "").trim();
  const ms = Date.parse(rawDate);
  return isNaN(ms) ? 0 : ms;
}

function comparePatientsDesc_(a, b) {
  const byId = extractPatientNumericOrderKey_(b) - extractPatientNumericOrderKey_(a);
  if (byId) return byId;
  const byDate = extractPatientDateOrderKey_(b) - extractPatientDateOrderKey_(a);
  if (byDate) return byDate;
  return String((a && a.nombre_completo) || "").localeCompare(String((b && b.nombre_completo) || ""));
}

function getRecentPatients_() {
  return allPatients.slice(0, RECENT_PATIENTS_LIMIT);
}
...
function updatePatientsTableInfo_(message) {
  ...
  if (info) info.innerText = String(message || "");
}
...
function buildPatientTableStatusMessage_() {
  ...
  if (patientListMode_ === "all") {
    return allPatients.length === 1
      ...
      : `Mostrando ${allPatients.length} pacientes`;
  }
  return `Mostrando los últimos ${Math.min(RECENT_PATIENTS_LIMIT, allPatients.length)} pacientes registrados`;
}

function renderPatientsTableForCurrentMode_() {
  const searchInput = document.getElementById("searchInput");
  const term = normalizePatientSearchTerm_(searchInput && searchInput.value);
  ...
  if (term) {
    const filtered = allPatients.filter(
      (p) =>
        (p.nombre_completo && String(p.nombre_completo).toLowerCase().includes(term)) ||
        (p.cedula && String(p.cedula).includes(term)),
    );
    renderTable(filtered);
    updatePatientsTableInfo_(`Encontradas ${filtered.length} coincidencias`);
    return;
  }
...
function resetPatientsQuickView_(options = {}) {
  ...
  patientListMode_ = "recent";
  selectedPatientSummaryId_ = "";

  const searchInput = document.getElementById("searchInput");
  if (searchInput && shouldClearSearch) searchInput.value = "";

  updatePatientsToggleButton_();
  if (shouldRender) renderPatientsTableForCurrentMode_();
}
...
function syncDoctorHomeInfographicMeta_() {
  ...
  if (!current) {
    ...
    if (btn) {
      btn.innerHTML = '<i class="fas fa-plus"></i> Crear infografía';
    }
    return;
  }
  ...
  if (btn) {
    btn.innerHTML = '<i class="fas fa-pen"></i> Editar infografía';
  }
}
...
function renderDoctorHomeInfographicEmptyState_(message) {
  ...
  if (empty) {
    ...
    empty.innerHTML = `
      ...
      <p>${escapeHtml_(message || "Aún no has creado una infografía para previsualizar desde inicio.")}</p>
    `;
  }
...
function renderDoctorHomeInfographicPreview_(list) {
  ...
  if (!feed.length) {
    renderDoctorHomeInfographicEmptyState_("Aún no has creado una infografía para previsualizar desde inicio.");
    return;
  }
...
function fetchAdminInfographicPosts_() {
  ...
  const loadPostsPromise = (window.vfDataBridge && window.vfDataBridge.getInfographicPostsAdmin)
    ? window.vfDataBridge.getInfographicPostsAdmin(requester)
    : postApiWithSession_({
...
function loadAdminHomeInfographicPreview_() {
  ...
  return fetchAdminInfographicPosts_()
    ...
    .catch((error) => {
      adminInfographicPosts_ = [];
      renderDoctorHomeInfographicEmptyState_((error && error.message) || "No se pudo cargar tu infografía.");
      return [];
    });
}
...
window.onDoctorInfographicPreviewAction = function(type, idPost) {
  ...
  if (type === "fuente") {
    ...
    if (!url) {
      showAdminPreviewNotice_("Esta publicación no tiene una fuente externa configurada.", "info");
      return;
    }
  ...
  if (type === "contacto") {
    ...
    if (!number) {
      showAdminPreviewNotice_("No tienes un teléfono de WhatsApp configurado en tu perfil.", "warning");
      return;
    }
...
function loadDoctorVacationStatus() {
  ...
  const loadVacationPromise = (window.vfDataBridge && window.vfDataBridge.getMyVacation)
    ? window.vfDataBridge.getMyVacation(requester)
    : postApiWithSession_({ action: "get_my_vacation", requester: requester });
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      if (!until || !title || !message) {
        alert("Completa fecha, título y mensaje.");
        return;
      }
      ...
      try {
        ...
        if (window.showToast) {
          window.showToast(
            res.warning
              ? "Vacaciones guardadas con advertencia de sincronización."
              : "Aviso de vacaciones actualizado.",
  ...
  if (btnClear) {
    btnClear.addEventListener("click", async () => {
      ...
      if (window.showToast) {
        window.showToast(
          res.warning
            ? "Vacaciones desactivadas con advertencia de sincronización."
            : "Aviso desactivado.",
          res.warning ? "warning" : "success"
        );
      }
...
function loadPatientsTable() {
  const tbody = document.getElementById("patientsTableBody");
  tbody.innerHTML =
    '<tr><td colspan="3" class="text-center">Actualizando base de datos...</td></tr>';

  const requester = getRequesterFromSession();
  postApiWithSession_({ action: "get_data", sheet: "pacientes", requester: requester })
    .then((response) => {
      if (response.success && response.data.length > 0) {
        allPatients = response.data.slice().sort(comparePatientsDesc_);
        renderPatientsTableForCurrentMode_();
        ...
        allPatients = [];
        tbody.innerHTML =
          '<tr><td colspan="3" class="text-center">No hay pacientes registrados.</td></tr>';
        updatePatientsToggleButton_();
        updatePatientsTableInfo_("No hay pacientes registrados.");
      }
    })
    .catch((error) => {
      console.error(error);
      tbody.innerHTML =
        '<tr><td colspan="3" class="text-center" style="color:red">Error de conexión.</td></tr>';
      updatePatientsTableInfo_("No se pudo cargar la lista de pacientes.");
    });
}
  ...
  dataArray.forEach((paciente) => {
    ...
    tr.innerHTML = `
            <td class="patient-cell patient-cell-cedula">
                <strong class="patient-cedula">${cedula}</strong>
            </td>
            ...
            <td class="patient-cell patient-cell-actions">
                <div class="patient-actions">
                    <button class="btn-icon" onclick="openPatientSummary('${patientId}')" title="Ver detalle">
                        <i class="fas fa-pencil-alt" style="color:#3498db;"></i>
                    </button>
                    
                    <button class="btn-icon" onclick="deletePatient('${patientId}')" title="Eliminar">
                        <i class="fas fa-trash" style="color:#e74c3c;"></i>
                    </button>

                    <button class="btn-icon" onclick="goToClinical('${patientId}')" title="Abrir Expediente">
...
window.openCreateModal = function () {
  closeModal("modalPatientSummary");
...
function buildPatientSummaryRow_(label, value) {
  return `
    <div style="padding:12px 14px; border:1px solid #e5e7eb; border-radius:10px; background:#fff;">
      <div style="font-size:0.8rem; color:#6b7280; margin-bottom:4px;">${escapeHtml_(label)}</div>
      <div style="font-weight:600; color:#1f2937;">${escapeHtml_(value || "-")}</div>
    </div>
  `;
}

window.openPatientSummary = function (id) {
  const paciente = allPatients.find((p) => p.id_paciente === id);
  const host = document.getElementById("patientSummaryBody");
  if (!paciente || !host) return;

  selectedPatientSummaryId_ = String(paciente.id_paciente || "").trim();
  const fechaNacimiento = paciente.fecha_nacimiento
    ? formatAdminDateLabel_(paciente.fecha_nacimiento, false)
    : "-";
  const fechaRegistro = paciente.fecha_registro
    ? formatAdminDateLabel_(paciente.fecha_registro, false)
    : "-";
  const antecedentes = paciente.antecedentes || paciente.antecedentes_medicos || "-";

  host.innerHTML = [
    buildPatientSummaryRow_("Cédula", paciente.cedula || "-"),
    buildPatientSummaryRow_("Nombre completo", paciente.nombre_completo || "-"),
    buildPatientSummaryRow_("Teléfono", paciente.telefono || "-"),
    buildPatientSummaryRow_("Correo", paciente.correo || "-"),
    buildPatientSummaryRow_("Fecha de nacimiento", fechaNacimiento),
    buildPatientSummaryRow_("Fecha de registro", fechaRegistro),
    buildPatientSummaryRow_("Dirección", paciente.direccion || "-"),
    buildPatientSummaryRow_("Ocupación", paciente.ocupacion || "-"),
    buildPatientSummaryRow_("Antecedentes", antecedentes || "-"),
  ].join("");

  openModal("modalPatientSummary");
};

window.editPatientFromSummary = function () {
  if (!selectedPatientSummaryId_) return;
  closeModal("modalPatientSummary");
  editPatient(selectedPatientSummaryId_);
};
...
window.editPatient = function (id) {
  ...
  if (inpAntecedentes) {
    inpAntecedentes.value = paciente.antecedentes || paciente.antecedentes_medicos || "";
  }
...
window.notifyPatientLoginFromModal = function () {
  ...
  if (!idPaciente) {
    alert("Esta opción solo aplica cuando editas un paciente.");
    return;
  }
  ...
  if (!waNumber) {
    alert("No se encontró un teléfono válido del paciente para WhatsApp.");
    return;
  }
  if (!cedula || !password) {
    alert("Faltan datos de login (cédula o contraseña).");
    return;
  }
  ...
  const msg =
    ...
    `Usuario: ${cedula}\n` +
    `Contrasena: ${password}\n\n` +
    `Por seguridad, puede cambiar su contraseña en su primer ingreso.\n` +
    `Atentamente,\nVIDAFEM`;
    ...
    .then((res) => {
      if (res.success) {
        notifyAdminMutationResult_(
          res,
          "Paciente eliminado.",
          "Paciente eliminado con advertencia de sincronización."
        );
        loadPatientsTable();
      } else {
        alert("Error: " + res.message);
...
if (formNewPatient) {
  formNewPatient.addEventListener("submit", function (e) {
    ...
    postApiWithSession_({
      ...
      .then((response) => {
        if (response.success) {
          const successMessage = idPaciente ? "Datos actualizados." : "Paciente registrado.";
          ...
          if (response.warning && window.showToast) {
            window.showToast(
              idPaciente
                ? "Paciente actualizado con advertencia de sincronización."
                : "Paciente registrado con advertencia de sincronización.",
              "warning"
            );
          }
          loadPatientsTable();
  ...
  links.forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      if (text === "Pacientes") {
        e.preventDefault();
        switchView("patients");
        resetPatientsQuickView_({ clearSearch: true, render: false });
        loadPatientsTable();
      }
      // Opción Inicio
      else if (text === "Inicio") {
...
window.togglePatientsListMode = function () {
  patientListMode_ = patientListMode_ === "all" ? "recent" : "all";
  renderPatientsTableForCurrentMode_();
};
...
function goToClinical(id) {
  ...
  window.navigateWithEnv(`clinical.html?id=${id}`);
}
...
function loadDashboardStats() {
  ...
  postApiWithSession_({ action: "get_dashboard_stats", requester: requester })
    .then((res) => {
      if (res.success) {
...
function openWeeklyAppointmentsModal_() {
  ...
  postApiWithSession_({
    ...
    .catch(() => {
      rangeEl.innerText = "No se pudo cargar la semana";
      listEl.innerHTML = '<p style="color:#c0392b; text-align:center;">Error de conexión al cargar las citas de la semana.</p>';
    })
    .finally(() => {
...
function loadServicesAdmin() {
  ...
  const loadServicesPromise = (window.vfDataBridge && window.vfDataBridge.getServices)
    ...
    : postApiWithSession_({ action: "get_services", requester: requester });
...
if (formService) {
  formService.addEventListener("submit", (e) => {
    ...
    postApiWithSession_({ action: "add_service", data: data })
      .then(() => {
...
if (formEditService) {
  formEditService.addEventListener("submit", (e) => {
    ...
    postApiWithSession_({ action: "update_service", data: data })
      .then((res) => {
        if (res.success) {
...
function loadPromoStatus() {
  ...
  const requester = getRequesterFromSession();
  const loadPromoPromise = (window.vfDataBridge && window.vfDataBridge.getPromoList)
    ? window.vfDataBridge.getPromoList(requester)
    : postApiWithSession_({ action: "get_promo_list", requester: requester });
...
window.deletePromo = function (id) {
  ...
  postApiWithSession_({ action: "delete_promotion", id: id, requester: getRequesterFromSession() })
    .then((res) => {
      if (!res || !res.success) {
...
if (formPromo) {
  formPromo.addEventListener("submit", (e) => {
    ...
    postApiWithSession_({ action: "save_promotion", data: data, requester: getRequesterFromSession() })
      .then((res) => {
        if (!res || !res.success) {
      ...
      <div style="padding:14px;">
        <div style="display:flex; justify-content:space-between; gap:10px; align-items:flex-start; flex-wrap:wrap;">
          <div>
            ...
            <h4 style="margin:6px 0; color:#2c3e50;">${p.titulo || "Sin título"}</h4>
...
function resetInfographicForm_() {
  ...
  if (btnInfo) btnInfo.value = "Más información";
  ...
  if (btn) btn.innerText = "Guardar publicación";
...
function fillInfographicForm_(post) {
  ...
  if (btnInfo) btnInfo.value = post.btn_info_text || "Más información";
  ...
  toggleInfographicActionFields_();
  if (btnSave) btnSave.innerText = "Actualizar publicación";
...
function loadInfographicPostsAdmin() {
  ...
  return fetchAdminInfographicPosts_()
    ...
    .catch((error) => {
      adminInfographicPosts_ = [];
      if (container) {
        container.innerHTML = `<p style="text-align:center; color:#c0392b;">${(error && error.message) || "Error de conexión."}</p>`;
      }
      renderDoctorHomeInfographicEmptyState_((error && error.message) || "No se pudo cargar tu infografía.");
      return [];
    });
    ...
    postApiWithSession_({
      ...
      .then((res) => {
        ...
        if (window.showToast) {
          window.showToast(
            res.warning
              ? "Infografia guardada con advertencia de sincronización."
      ...
      .finally(() => {
        if (btnSave) {
          btnSave.disabled = false;
          btnSave.innerText = oldText || "Guardar publicación";
        }
      });
  });
}
...
window.deleteInfographicPost = function(idPost) {
  if (!confirm("¿Eliminar esta publicación visual?")) return;
  postApiWithSession_({
    ...
    .then((res) => {
      ...
      if (window.showToast) {
        window.showToast(
          res.warning
            ? "Infografia eliminada con advertencia de sincronización."
...
window.openServiceBuilder = function(existingService = null) {
    ...
    if (existingService) {
        ...
        const loadConfigPromise = (window.vfDataBridge && window.vfDataBridge.getServiceConfig)
            ...
            : postApiWithSession_({ action: "get_service_config", requester: requester });
        loadConfigPromise
        .then(res => {
    ...
    try {
        ...
        if(res.success) {
            if (window.showToast) {
                window.showToast(
                    res.warning
                        ? "Servicio guardado con advertencia de sincronización."
                        : "Servicio guardado correctamente.",
                    res.warning ? "warning" : "success"
                );
            } else if (res.warning) {
                alert("Servicio guardado. Advertencia: " + res.warning);
            } else {
...
window.deleteCurrentService = function() {
    ...
    if(confirm("¿ESTÁS SEGURO\nSe borrará el servicio '" + name + "' y toda su configuración.")) {
        postApiWithSession_({ action: "delete_service_full", nombre: name, requester: getRequesterFromSession() })
        .then(res => {
            if(res.success) {
                if (window.showToast) {
                    window.showToast(
                        res.warning
                            ? "Servicio eliminado con advertencia de sincronización."
                            : "Servicio eliminado.",
                        res.warning ? "warning" : "success"
                    );
                } else if (res.warning) {
                    alert("Servicio eliminado. Advertencia: " + res.warning);
                } else {
                    alert("Eliminado.");
                }
                closeModal('modalServiceBuilder');
