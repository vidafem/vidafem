...
async function saveCommon(tipo, generarPdf, btnClicked, getDataFn) {
  ...
  try {
    ...
    if (tipo === "EXAMENPDF") {
      const hasNewPdf = !!(specificData.pdf_externo && specificData.pdf_externo.data);
      const hasExistingPdf = !hasNewPdf && !window.pdfExternoEliminado && hasExistingExternalPdfLoaded_();
    ...
    if (res.success) {
      ...
      if (generarPdf && pdfWindow) {
          ...
          const externalPdfOnly = tipo === "EXAMENPDF" ? String(res.pdf_externo_url || "").trim() : "";
          if (externalPdfOnly) {
              pdfWindow.location.href = externalPdfOnly;
          } else if (recipeOnlyPdf) {
...
function hasExistingExternalPdfLoaded_() {
    return !!document.getElementById("existingPdfMsg");
}
...
function getExternalPdfPayloadForSave_() {
    ...
    if (!container.classList.contains("hidden")) {
        if (typeof getPdfExternoData === "function") return getPdfExternoData();
...
function saveExternalPdfOnly(generarPdf, btn) {
    saveCommon("EXAMENPDF", generarPdf, btn, async () => {
        const pdfFile = await getExternalPdfPayloadForSave_();
        return {
            datos_json: {
...
async function saveDynamicService(servicio, generatePdf, btn, recetaData) {
    ...
    try {
        ...
        const hasExternalPdf = !!(pdfFile && pdfFile.data);
        const keepsExternalPdf = !hasExternalPdf && !window.pdfExternoEliminado && hasExistingExternalPdfLoaded_();

        if (!hasDynamicContent) {
            if (hasRecipeContent && (hasExternalPdf || keepsExternalPdf)) {
                throw new Error("Este servicio quedaria sin contenido clinico. Usa RECETA MEDICA si solo emitiras receta, o EXAMENPDF si solo adjuntaras un examen.");
            }
            if (hasRecipeContent) {
                throw new Error("Este servicio quedaria sin contenido clinico. Si solo vas a emitir una receta, usa la opcion RECETA MEDICA.");
            }
            if (hasExternalPdf || keepsExternalPdf) {
                throw new Error("Este servicio quedaria sin contenido clinico. Si solo vas a adjuntar un examen, usa la opcion EXAMENPDF.");
            }
