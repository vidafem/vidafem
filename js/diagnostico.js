...
function buildDiagnosisTemplateImagesHtml_(images) {
  ...
  return ""
    ...
    + "<div style=\"display:flex;flex-wrap:wrap;gap:2.4mm;align-items:flex-start;justify-content:center;\">"
    + blocks
    + "</div>"
...
function ensureSignModalsExist() {
    ...
    function onMove(e) { if (!isDragging) return; const touch = e.touches ? e.touches[0] : e; const contRect = container.getBoundingClientRect(); let newL = touch.clientX - contRect.left - offsetX; let newT = touch.clientY - contRect.top - offsetY; el.style.left = newL + 'px'; el.style.top = newT + 'px'; const mB = container.closest('.modal-body'); if (mB) { const bR = mB.getBoundingClientRect(); if (touch.clientY > bR.bottom - 60) mB.scrollTop += 25; else if (touch.clientY < bR.top + 60) mB.scrollTop -= 25; } }
